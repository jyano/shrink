...
ct.N=function(n){var s=this; if(U(n)){return s.nextStage}; s.nextStage=n; return s}
...
ct.Tx = ct.text = function (t, f, c, x, y) {
    var ct = this, o, cX
    cX = this.st().cen().x
    o = (N(x) && U(y)) ? {y: x, x: cX} : U(x) ? {x: cX, y: 100} : {t: t, f: f, c: c, x: x, y: y}
    return cjs.T(o.t, o.f, o.c, o.x, o.y).a2(ct)
}
ct.cX = function () {
...
//ct.cir = function (c, r, x, y) {var g=G(arguments); return $Cir(c, r, x, y).a2(this)}
ct.cir = function (x, y, r, c) {this.A(cjs.cir(x, y, r, c));return this}
...
ct.ct = function (x, y) {var ct = this, g = G(arguments), x = g[0], y = g[1]
    nCt = $Ct().a2(ct)
    if (N(g[0])) {nCt.XY(g[0], g[1])}
    if (F(g[0])) {g[0](nCt, ct)}
    if (g.p) {nCt.SL()}
    return nCt
}
ct.bData = function (data) {
    var a = cjs.bm($.i().src($.parseJSON(data)))
    this.A(a)
    return a
}
...
ct.bgI = ct.bg = function (i) {
    var ct = this;
    ct.bm(i, function (b) {ct.setChildIndex(b, 0)
...
ct.auCl = ct.noAutoClear = ct.noAuCl= function () {
